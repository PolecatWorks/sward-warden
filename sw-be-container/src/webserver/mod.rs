...
pub fn app_router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            ...
            get(list_organic_manure_applications).post(create_organic_manure_application),
        )
        .route(
            "/v0/compliance-breaches",
            get(list_compliance_breaches).post(create_compliance_breach),
        )
        .route(
            "/v0/sward-movements",
            get(list_sward_movements).post(create_sward_movement),
        )
...
async fn delta_sync(
    ...
    let organic_manure_applications = sqlx::query_as::<_, OrganicManureApplication>(
        "SELECT oma.id, oma.event_id, oma.manure_type, oma.volume_applied_m3_per_ha, oma.weight_applied_tonnes_per_ha, oma.nitrogen_content_kg_per_unit, oma.is_lesse_applied, oma.weather_conditions_confirmed, oma.buffer_zone_distance_meters, oma.updated_at, oma.is_deleted, oma.equipment_used, oma.lesse_exemption_reason FROM organic_manure_applications oma JOIN events e ON oma.event_id = e.id JOIN fields f ON e.field_id = f.id JOIN farms far ON f.farm_id = far.id WHERE far.user_id = 1 AND oma.updated_at > $1"
    )
    .bind(since)
    .fetch_all(&state.db_pool)
    .await?;
