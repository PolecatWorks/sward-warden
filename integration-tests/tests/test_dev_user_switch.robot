...
Dev User Switching Flow
    ...
    # 2. Login as Demo User (ID 1)
    Click    css=[data-testid="user-login-1"]
    ...
    # 6. Switch back to Demo User via dropdown using value=1 (robust to name changes)
    Run Keyword And Ignore Error    Select Options By    id=user-switcher-dropdown    value    1
...
Verify Force Sync With User Change In Between
    ...
    # 1. Login as Demo User (ID 1)
    New Page    ${EXTERNAL_DNS_URL}/login
    Wait For Elements State    css=[data-testid="user-login-1"]    visible    timeout=10s
    Click    css=[data-testid="user-login-1"]
    Wait For Elements State    css=app-home    visible    timeout=10s
