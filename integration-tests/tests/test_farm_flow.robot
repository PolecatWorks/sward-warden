...
*** Test Cases ***
# PRD Reference: 0003
Farm Creation and Deletion Flow - Save Button Click
    [Documentation]    Test farm creation via Save button and deletion flow end-to-end
    ...
    ${farm_name}=    Set Variable    E2E Test Farm Btn ${random_str}
    ${farm_location}=    Set Variable    E2E Test Location Btn
    ...
    # 4. Check farm exists via API and verify ONLY ONE farm was created
    ...
    FOR    ${farm}    IN    @{farms}
        ...
        END
    END
    Should Be Equal As Integers    ${found_count}    1    Expected exactly 1 farm, but found ${found_count}
...
# PRD Reference: 0003
Farm Creation and Deletion Flow - Enter Key
    [Documentation]    Test farm creation via Enter key and deletion flow end-to-end
    [Teardown]    Teardown With Video
    New Browser    chromium    headless=True
    New Context    recordVideo={"dir": "${OUTPUT_DIR}/videos"}
    Login As Demo User
    ...
    # Generate a unique farm name
    ${random_str}=    Evaluate    str(random.randint(1000, 9999))    modules=random
    ${farm_name}=    Set Variable    E2E Test Farm Ent ${random_str}
    ${farm_location}=    Set Variable    E2E Test Location Ent
    ...
    # 3. Press Enter key inside location input to submit
    Keyboard Key    press    Enter

    # Wait for the add farm modal to close
    Wait For Elements State    \#save-farm-btn    detached    timeout=5s

    # 4. View the created farm in UI
    Wait For Elements State    text=${farm_name}    visible    timeout=10s

    # Wait for sync
    Sleep    5s

    # 5. Check farm exists via API and verify ONLY ONE farm was created
    ${list_response}=    GET    ${BE_BASE_URL}/v0/farms    expected_status=200
    ${farms}=    Set Variable    ${list_response.json()}
    ...
    FOR    ${farm}    IN    @{farms}
        ...
        END
    END
    Should Be Equal As Integers    ${found_count}    1    Expected exactly 1 farm via Enter, but found ${found_count}

    # 6. Navigate to the farm detail view page
    Go To    ${EXTERNAL_DNS_URL}/farms/${farm_id}
    Wait For Elements State    id=farm-name-heading    visible    timeout=5s
    ...
    # 9. Confirm deleted in UI (wait until text is not present)
    Wait For Elements State    text=${farm_name}    detached    timeout=10s

    # Wait for sync
    Sleep    5s

    # 10. Confirm deleted via API
    ${list_response_after}=    GET    ${BE_BASE_URL}/v0/farms    expected_status=200
    ${farms_after}=    Set Variable    ${list_response_after.json()}
...
Farm Deletion Blocking and Field Migration Flow
    ...
    Click    \#save-edit-field-btn    button=left
