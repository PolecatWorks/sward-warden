# 0020-03 Drop X-User-ID Header Support

## Description
This specification resolves the contradiction identified in PRDs regarding the use of the `X-User-ID` header versus the `Authorization: Bearer <token>` JWT flow. As dictated by PRD 0020, the application must fully migrate to using JWTs for authentication and drop legacy support for `X-User-ID` headers in frontend API calls.
