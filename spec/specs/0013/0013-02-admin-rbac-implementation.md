# Spec 0013-02: Be Role-Based Access Control (RBAC)

## Status: Complete

## 1. Overview
The Administration Console requires specific permissions. This spec details the implementation of `Admin` and `Support` roles in the be to restrict access to administrative APIs.

## 2. Requirements
- Extend the User model or Auth context to include roles.
- Implement middleware/guards in the be to check for `Admin` or `Support` roles.
...
- [ ] Update fe `AuthService` to handle roles and protect admin routes.
