# Specification 0001-10: RxDB Local Database Setup

## Goal
Integrate RxDB into the Angular fe as the local-first data layer. Define local schemas mirroring the be PostgreSQL models and rewire Angular services to read/write from RxDB instead of making direct HTTP calls.
...
4. **Service Layer Refactoring**:
   ...
   - All FE reads should subscribe to RxDB reactive queries (observables) so the FE updates immediately on local writes.
   - HTTP calls to the be API should be removed from the direct read/write path; they will be reintroduced in specs 0011-03 and 0011-04 as part of the sync layer.
5. **Testing**:
...
- FE components continue to function identically (no regressions) using local data.
