# Spec 0002-13: Adaptive Layouts

## 1. Description
This specification details the implementation of adaptive layouts across mobile, tablet, and desktop viewports, as derived from PRD 0002. The goal is to ensure the FE gracefully scales to utilize available screen real estate effectively without compromising readability or usability.

## 2. Acceptance Criteria
1. **Mobile Layout (`< 640px` / default Tailwind classes)**:
   - The FE must continue to use the existing mobile-first layout.
...
3. **Desktop Layout (`lg:` and up / `>= 1024px`)**:
   - The FE must fully utilize desktop screen width using multi-column layouts for dashboards and data entry forms.
...
1. Add a wrapper container in the main application layout file (e.g., `app.component.html` in the Angular fe) applying the max-width constraint (e.g., `<div class="mx-auto max-w-7xl w-full">`).
