...
- **In-Test Resource Teardown:** All integration test cases creating test entities (Users, Farms, Fields, Events, Soil Analyses, Storage Capacities) must register created entity IDs and clean them up via `[Teardown]` hooks or explicit API delete calls upon test completion (whether passed or failed).
- **Stale & Orphaned Test Data Cleanup Mechanism:** Provide a automated cleanup suite/script that can be run on demand or post-testing to identify and sweep orphaned or leftover test-generated entities (e.g. entities matching test prefixes such as `Test Farm`, `Test User`, or `robot_*`) across the backend API.
