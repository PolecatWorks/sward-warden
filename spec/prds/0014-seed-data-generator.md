...
## 1. Introduction
The objective of this project is to create a developer-oriented tool that generates realistic, localised seed data for the Sward Warden backend API and database. This tool will simplify development, testing, and demonstration by populating the database with representative data without requiring manual data entry.

## 2. Goals & Objectives
*   Provide a fast and repeatable way to populate the Sward Warden database with mock data.
*   Ensure the generated data conforms to the backend API schema.
...
*   Integration with an HTTP client (e.g., `reqwest` crate) to interact with the Sward Warden backend API.
...
*   Inclusion of the tool in the main backend production Docker image or release binaries.
...
*   **REQ-2**: The tool must communicate with the backend API over HTTP to create resources.
...
*   **NFR-1 (Maintainability)**: The tool should be written in Rust to keep the toolchain consistent with the backend repository.
...
*   The `reqwest` crate is suitable for making async HTTP requests to the backend API endpoints.
*   Since the backend API currently uses paths like `/v0/users` and `/v0/farms`, the tool must construct requests targeting these routes.
*   Error handling should be implemented to gracefully report issues if the backend API is unreachable or returns errors during data creation.
