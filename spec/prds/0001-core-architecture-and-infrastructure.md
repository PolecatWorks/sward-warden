...
- **Sync Error Interception & Graceful Recovery:** When synchronization network calls (push/pull sync) return HTTP error statuses (specifically HTTP `403 Forbidden` or authorization errors), the frontend must catch these errors gracefully without crashing the app, dropping local state, or triggering unhandled app resets. The app must transition to a dedicated Sync Error state and navigate to the dedicated error page (`/error`), present the error details along with the logged-in user's identity info, display a visual retry countdown progress bar, and automatically retry sync upon timer expiration. Upon receiving a successful sync response (`200 OK`) on retry, the application must seamlessly clear the error state and resume normal operation.
- **Service Worker Navigation Scope & Path Bypasses:** The frontend Service Worker (`ngsw-worker.js`) configuration (`ngsw-config.json`) must explicitly declare `navigationUrls` exclusions (`!/auth/**`, `!/api/**`, `!/hams/**`). This ensures that browser navigation and HTTP requests to Keycloak authentication (OIDC discovery `.well-known`, realm logins), backend APIs, and health probes bypass PWA Service Worker caching and route directly to the backend/network, preventing Service Worker `index.html` fallback loops.
