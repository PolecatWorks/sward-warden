...
- **Sync Error Interception & Graceful Recovery:** When synchronization network calls (push/pull sync) return HTTP error statuses (specifically HTTP `403 Forbidden` or authorization errors), the frontend must catch these errors gracefully without crashing the app, dropping local state, or triggering unhandled app resets. The app must transition to a dedicated Sync Error state, present the error details along with the logged-in user's identity, display a visual retry countdown progress bar, and automatically retry sync upon expiration. Upon receiving a successful sync response (`200 OK`) on retry, the application must seamlessly resume normal operation.
