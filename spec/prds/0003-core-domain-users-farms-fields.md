...
- **Field Creation and Deletion Journey (`test_field_flow.robot`)**: The integration testing suite must include an end-to-end field management journey. The journey must select an existing farm, use the UI to add a field with a specified name and area, and verify that the field appears in the UI list. It must confirm via the backend API that exactly one field is created, navigate to the field detail page in the UI, delete the field, and verify that the field is removed from both the UI and the backend API.
