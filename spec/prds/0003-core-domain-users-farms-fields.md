...
- **Field Topology Creation and Mapping Verification Journey (`test_field_mapping_flow.robot`)**: The integration testing suite must include a spatial mapping validation journey. The journey must navigate the UI to create a new field and interact with the mapping interface to define a custom polygonal boundary (e.g., using 4 corners) situated in the Ballycastle area of Northern Ireland. After successfully capturing the field location and saving, the journey must open the field detail view and observe the field on the map, verifying that the map interface clearly displays the field bounded exactly by the custom polygonal boundary defined during creation.
