...
- **Keycloak User Identity & First-Time Login Onboarding:** The user ID is provided directly by Keycloak as a long string UUID (the `sub` claim in the decoded JWT token, e.g. `ae5245cd-3095-46db-8ce3-cea42fe26edf`). On initial login, the application attempts to fetch the user profile via `GET /sward/v0/users/{userId}` (e.g. `https://sw-dev.polecatworks.com/sward/v0/users/ae5245cd-3095-46db-8ce3-cea42fe26edf`). If the profile does not exist (HTTP 404 response on first login), the user must be automatically navigated to an edit user screen to enter and submit their profile details. Form fields on the edit user screen must be pre-populated with available information extracted directly from the decoded Keycloak JWT token (including `name`, `given_name`, `family_name`, `email`, and `preferred_username`) to streamline onboarding before creating the user record in the backend.
- **User Creation & ID Sequence Behavior (`POST /v0/users`):**
  - **Explicit ID Creation (`user.id > 0`):** When an explicit user ID is provided (e.g., during test suite setup or admin seeding), the backend attempts a plain `INSERT INTO users (id, ...)` within an atomic transaction. If insertion fails for any reason (such as a primary key or unique email constraint conflict), the backend does **NOT** attempt automatic resolution or sequence manipulation and immediately returns the database error to the caller.
  - **Auto-Increment Sequence Creation (`user.id <= 0`):** When no explicit ID is provided, the backend inserts the user using the database auto-increment sequence (`users_id_seq`). If a primary key collision (`users_pkey`) occurs due to sequence counter drift against previously inserted explicit IDs, the backend dynamically advances the sequence using `SELECT nextval(...)` inside a retry loop within the active transaction until a free ID is secured.
  - **Atomic Transaction Scoping:** User record creation, sequence retries, module assignments (`user_modules`), and fetching the populated user representation must execute within a single atomic database transaction.
...
- **Safe Deletion:** Farms can only be deleted from the Farm Details page (`/farms/:farmId`) while in the edit view (inside the edit modal) via a red trash can icon/button revealing an inline confirmation. Deletion is blocked (FE & BE) if active fields remain attached.
...
- **Area Calculation Journey (`test_field_topology.robot`)**: The integration testing suite must test the area calculation by creating a field with a polygon, clicking the Edit button, clicking the "Calculate Area" button, confirming the area input updates with the expected value, saving the field, and verifying the area update via API.
