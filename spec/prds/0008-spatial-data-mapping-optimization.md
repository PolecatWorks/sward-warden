...
## 7. User Journeys
- **Field Topology Creation and Spatial Mapping Journey (`test_field_topology.robot`)**: The integration testing suite must include a comprehensive spatial data mapping journey testing both API functionality and the UI frontend interface. The journey must verify creating fields with various topologies (Polygon GeoJSON geometry, Point GeoJSON geometry, and null/no geometry) via both the API and UI map drawing tools (including address searching for Ballycastle, Northern Ireland, and drawing polygon boundary corners). It must ensure that the map tools accurately create and verify these locations on the backend API and visualise them on the UI.
