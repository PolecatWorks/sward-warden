...
- **Testing before Push**: Before pushing changes to the repository, you MUST run the integration tests using `make robot-test` and confirm that all tests are passing successfully. However, robot tests are NOT required if the backend (`sw-be-container/`) and frontend (`sw-fe-container/`) code are not directly modified.
- **Docker Image Builds**:
