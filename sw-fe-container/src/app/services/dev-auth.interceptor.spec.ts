...
describe('devAuthInterceptor', () => {
  ...
  it('should not add a fake JWT if Authorization header is present', () => {
    httpClient.get('/test-endpoint', { headers: { 'Authorization': 'Bearer real-token' } }).subscribe();
  ...
  it('should navigate to /error with correct state on 401 response', () => {
    ...
    const req = httpMock.expectOne('/test-endpoint');
    req.flush({ error: 'Unauthorized Access' }, { status: 401, statusText: 'Unauthorized' });

    // PRD Reference: 0020
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/error'], { state: { error: 'Unauthorized Access' } });
  });
  ...
  it('should navigate to /error with correct state on 403 response', () => {
    ...
    const req = httpMock.expectOne('/test-endpoint');
    req.flush({ error: 'Forbidden Access' }, { status: 403, statusText: 'Forbidden' });

    // PRD Reference: 0020
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/error'], { state: { error: 'Forbidden Access' } });
  });
  ...
  it('should navigate to /error with default message on 401/403 if no error message provided in response body', () => {
    ...
    // PRD Reference: 0020
    expect(routerSpy.navigate).toHaveBeenCalledWith(['/error'], { state: { error: 'Authentication failed. Please check your credentials or access rights.' } });
  });
